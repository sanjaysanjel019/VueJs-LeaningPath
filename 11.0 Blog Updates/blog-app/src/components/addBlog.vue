<template>
  ...
  <form v-if="!submitted">
    ...
    <div id="checkboxes">
      ...
      <label>Pro</label>
      <input type="checkbox" value="pro" v-model="blog.categories"/>
    ...
    <select v-model="blog.author">
      <option v-for="(auth,id) in authors" v-bind:key="id">{{auth}}</option>
    </select>
...
<div id="preview">
  ...
  <p>Blog Content:</p>
  <p>{{blog.content}}</p>
  <p>Blog  Category: {{blog.categories}}</p>
...
export default {
  ...
  data:function (){
    return{
     ...
     authors:['Jason Haddix','Paulo Coelho','Markus Zusak'],
     submitted:false
    }
  ...
  methods:{
    post: function(){
      this.$http.post('https://vue-net-ninja-4c80a.firebaseio.com/posts.json', this.blog).then(function(data){
